...
describe('parseSingleReference', () => {
	...
	it('should fail if there is trailing text', () => {
		const result = parseSingleReference('テトス1:14abc', aliases);
		expect(result).toBeNull();
	});

	it('should fail for out-of-range references when verseMap is provided', () => {
		const result = parseSingleReference('テトス9:99', aliases, 0, verseMap);
		expect(result).toBeNull();
	});

	it('should pass for valid references when verseMap is provided', () => {
		const result = parseSingleReference('テトス1:14', aliases, 0, verseMap);
		expect(result).not.toBeNull();
	});
});
